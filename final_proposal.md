...
urban environments. In New York City, hundreds of crashes are recorded
every day, reflecting patterns in traffic volume, street design, driver
behavior, and policy initiatives such as *Vision Zero*. By analyzing
**when** and **where** crashes occur and **which factors** contribute to
severe outcomes, this project aims to generate actionable insights for
traffic safety and urban planning. Using the NYC Open Data dataset
*“Motor Vehicle Collisions – Crashes”*, we will explore temporal,
spatial, and causal patterns in collisions across New York City as well
as indicators for injury, fatality, and grouped contributing factors.
...
Our analysis uses the NYC Open Data *“Motor Vehicle Collisions –
Crashes”* dataset, which contains detailed information on all reported
traffic collisions in New York City since 2012. Each record includes the
date and time of the crash (`CRASH_DATE`), the borough and location
details (`BOROUGH`, `ZIP_CODE`), the number of people injured or killed
(`NUMBER_OF_PERSONS_INJURED`, `NUMBER_OF_PERSONS_KILLED`), and the
contributing factors and vehicle types involved
(`CONTRIBUTING_FACTOR_VEHICLE_1`, `VEHICLE_TYPE_CODE_1`). These
variables allow us to examine temporal, spatial, and behavioral trends
in crash risk and severity across the five NYC boroughs.
...
- **Week 1 (Proposal + Setup):** Finalize questions and roles, set up
  GitHub repo, clean a subset of data.  
- **Week 2 (EDA + Visuals):** Summarize by borough, time, severity; make
  trend and heatmap plots.  
- **Week 3 (Spatial & Factor Analysis):** Map crash density, analyze
  contributing factors and vehicle types.  
- **Week 4 (Modeling + Draft):** Fit models for injury/fatality risk,
  draft report and integrate visuals.  
- **Week 5 (Polish + Website):** Finalize report and site, record
  screencast, check reproducibility.
