...
We are interested in whether there's death in collision using logistic regression

```{r logistic regression}
set.seed(123)
# use a small dataset to do regression
mvc_logit <- mvc_model |>
  sample_n(5000)  

logit_mod <- glm(
  I(number_of_persons_killed > 0) ~ borough + vehicle_type_code_1 + contributing_factor_vehicle_1,
  data   = mvc_logit,
  family = binomial()
)
...
logit_or <- tidy(logit_mod, exponentiate = TRUE, conf.int = TRUE)
...
logit_or |>
  ...
  coord_flip() +
  labs(
    title = "Logistic Model: Odds Ratios for Fatal Collision",
    x = "Predictor",
    y = "Odds Ratio (95% CI)"
  ) +
